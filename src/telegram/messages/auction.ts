...
export const formatAuctionMessage = (auction: Auction): string => {
  ...
  return `*${statusIcon} Auction Name:* ${escapeMarkdown(auction.name)}\n` +
         ...
         `*🚦 Status:* ${statusText}\n` +
         `*🗓️ Start Date:* ${new Date(auction.startDate).toLocaleString().replace(/\./g, '\\.')}\n` +
         `*⏰ End Date:* ${new Date(auction.endDate).toLocaleString().replace(/\./g, '\\.')}\n` +
         `*📅 Created On:* ${new Date(auction.createdDate).toLocaleString().replace(/\./g, '\\.')}\n` +
         `*🔄 Last Updated:* ${new Date(auction.updatedDate).toLocaleString().replace(/\./g, '\\.')}`;
};
